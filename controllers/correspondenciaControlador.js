...
const buscarCorrespondencia = async (req, res) => {
    try {
        ...
        // Filtro de texto libre en los campos especificados
        if (texto && texto.trim() !== "") {
        ...
        // Filtro por rango de fechas (fecha_envio)
        if (fechaInicio || fechaFin) {
            const desde = fechaInicio ? new Date(fechaInicio) : new Date("1700-01-01");
        ...
        // Filtros directos
        if (autor) filtros.autor = new RegExp(autor.trim(), "i");
        if (ciudad) filtros.ciudad = new RegExp(ciudad.trim(), "i");
        if (destinatario) filtros.destinatario = new RegExp(destinatario.trim(), "i");

        console.log("Filtros aplicados:", filtros); // Log para verificar los filtros

        const resultados = await correspondencia.find(filtros).limit(50);
        ...
        console.error("❌ Error en la búsqueda:", error); // Log del error completo
        return res.status(500).json({
            status: "error",
