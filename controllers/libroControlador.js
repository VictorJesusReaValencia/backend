...
const buscarLibros = async (req, res) => {
    try {
        const { texto, anioInicio, anioFin, fecha_publicacion, ciudad, editorial, isbn } = req.query;

        const filtros = {};

        // Filtro de texto libre (aplicado a autor, ciudad, editorial e isbn)
        if (texto && texto.trim() !== "") {
        ...
        // Filtro por rango de años (anioInicio - anioFin)
        ...
        // Filtro directo por ciudad
        if (ciudad) filtros.ciudad = new RegExp(ciudad, "i");

        // Filtro directo por editorial
        if (editorial) filtros.editorial = new RegExp(editorial, "i");

        // Filtro directo por ISBN
        if (isbn) filtros.isbn = new RegExp(isbn, "i");

        const resultados = await libros.find(filtros).limit(50);
