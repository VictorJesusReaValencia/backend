...
const registrarFotografia = async (req, res) => {
    ...
    try {
        ...
        const nombrePeriodico = limpiarTexto(doc.titulo);
        ...
        for (let archivo of archivos) {
            ...
            let baseName = `Documentacion_${nombrePeriodico}`;
...
const registrarPDF = async (req, res) => {
    ...
    try {
        ...
        const nombrePeriodico = limpiarTexto(libro.titulo);
        ...
        for (let archivo of archivos) {
            ...
            let baseName = `Documentacion_PDF_${nombrePeriodico}`;
            if (baseName.length > 50) {
                baseName = baseName.slice(0, 50);
            }
...
const buscarDocumentacion = async (req, res) => {
    try {
        const { texto, fecha_adquisicion, institucion, pais, ciudad, autor, titulo } = req.query;
        ...
        const resultados = await documentacion.find(filtros).limit(50);
