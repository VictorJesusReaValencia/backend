...
const registrarFotografia = async (req, res) => {
    ...
    try {
        const doc = await objetos.findById(id);
        if (!doc) {
...
const registrarPDF = async (req, res) => {
    ...
    try {
        const libro = await objetos.findById(librosId);
        if (!libro) {
            return res.status(404).json({
                status: "error",
                message: "Objetos no encontrado"
            });
        }
        ...
        for (let archivo of archivos) {
            ...
            let baseName = `Objetos_PDF_${titulo}`;
            if (baseName.length > 50) {
                baseName = baseName.slice(0, 50);
            }
...
const borrarPdfs = async (req, res) => {
    ...
    try {
        ...
        await objetos.findByIdAndDelete(id);

        return res.status(200).json({
            status: "success",
            message: "Objetos, imágenes y PDFs eliminados correctamente"
        });
        ...
        return res.status(500).json({
            status: "error",
            message: error.message || "Error al borrar el objeto"
        });
    }
};
...
const editarObjetos = async (req, res) => {
    ...
    try {
        let foto = await objetos.findByIdAndUpdate(id, datosActualizados, { new: true });
...
const buscarObjetos = async (req, res) => {
    try {
        const { texto, pais, institucion, ciudad, autor } = req.query;
        ...
        if (institucion) filtros.institucion = new RegExp(institucion, "i");
        if (ciudad) filtros.ciudad = new RegExp(ciudad, "i");
        if (autor) filtros.autor = new RegExp(autor, "i");

        const resultados = await objetos.find(filtros).limit(50);
